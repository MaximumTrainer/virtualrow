...
scenery/
  ...
  manifest.json  Every shipped model's size, generated by
                 scripts/generate-scenery-manifest.mjs (issue #332)
```

Multi-view PNG previews of each model live in `assets-src/scenery/renders/`,
beside the STEP files. They were in here until issue #332: vite copies `public/`
into `dist/` verbatim, so 133 previews - 39 MB, referenced by nothing in the app
- were published to every browser that loaded one 90 kB tree.

The GLBs are meshopt-compressed and quantised (`npm run build:assets`). Nothing
in this folder is hand-edited; `scripts/build_scenery.py` generates it.
