...
export interface StartSequenceInput {
  ...
  active: boolean;
  /** The rower has taken a stroke. Read only while armed. */
  strokeSeen: boolean;
...
export function useStartSequence({ active, strokeSeen, autoStart }: StartSequenceInput): StartSequence {
  const [state, setState] = useState<State>({ phase: 'idle', countdown: null, run: 0 });
  ...
  if (!active) {
    ...
    if (next.phase === 'armed' && (strokeSeen || autoStart)) {
      next = { phase: 'counting', countdown: COUNTDOWN_FROM, run: state.run + 1 };
    }
  }
  if (next !== state) setState(next);

  // The whole sequence is scheduled when the count starts, rather than each
  // step from the render the last one caused, so a slow frame cannot stretch it.
  ...
  useEffect(() => {
    ...
    const steps: Array<[number, Omit<State, 'run'>]> = [];
    ...
    const ids = steps.map(([at, step]) =>
      setTimeout(() => setState((s) => (s.run === run ? { ...step, run } : s)), at),
    );
