...
const RowerScene: React.FC<Rower3DProps> = ({ route, paceSPer500, distanceMeters, isPlaying, cadence, performanceMode, intensityFactor }) => {
  ...
  const isLondonRoute = useMemo(() => {
    return route.name?.toLowerCase().includes('thames') || route.name?.toLowerCase().includes('london') || route.tags?.includes('london') || false;
  }, [route.name, route.tags]);
  ...
  // Generate River Thames London specific landmarks - Big Ben, Tower Bridge, London Eye, etc.
  const londonLandmarks = useMemo(() => {
    if (!isLondonRoute) return null;
  ...
  return (
    <>
      ...
      {/* RIVER THAMES LONDON SPECIFIC LANDMARKS */}
      {isLondonRoute && londonLandmarks && (
        <group>
          {/* Big Ben / Elizabeth Tower and Houses of Parliament */}
          <group position={[londonLandmarks.bigBen.x, 0, londonLandmarks.bigBen.z]}>
            {/* Elizabeth Tower (Big Ben) */}
            <mesh position={[0, londonLandmarks.bigBen.towerHeight / 2, 0]}>
              <boxGeometry args={[londonLandmarks.bigBen.towerWidth, londonLandmarks.bigBen.towerHeight, londonLandmarks.bigBen.towerWidth]} />
              <meshStandardMaterial color={'#c9a227'} roughness={0.7} />
            </mesh>
            {/* Tower spire */}
            <mesh position={[0, londonLandmarks.bigBen.towerHeight + 3, 0]}>
              <coneGeometry args={[1.5, 6, 8]} />
              <meshStandardMaterial color={'#1e3a5f'} roughness={0.6} />
            </mesh>
            {/* Clock face (front) */}
            <mesh position={[londonLandmarks.bigBen.towerWidth / 2 + 0.01, londonLandmarks.bigBen.towerHeight * 0.75, 0]}>
              <circleGeometry args={[londonLandmarks.bigBen.clockSize, 16]} />
              <meshStandardMaterial color={'#f8fafc'} roughness={0.3} />
            </mesh>
            {/* Clock hands */}
            <mesh position={[londonLandmarks.bigBen.towerWidth / 2 + 0.02, londonLandmarks.bigBen.towerHeight * 0.75, 0]}>
              <boxGeometry args={[0.1, londonLandmarks.bigBen.clockSize * 0.8, 0.1]} />
              <meshStandardMaterial color={'#1f2937'} roughness={0.3} />
            </mesh>
            {/* Houses of Parliament - main building */}
            <mesh position={[8, londonLandmarks.bigBen.parliamentHeight / 2, 0]}>
              <boxGeometry args={[londonLandmarks.bigBen.parliamentLength, londonLandmarks.bigBen.parliamentHeight, 6]} />
              <meshStandardMaterial color={'#c9a227'} roughness={0.75} />
            </mesh>
            {/* Parliament towers/spires */}
            {[-3, 5, 12, 18].map((xOffset, i) => (
              <mesh key={`parliament-tower-${i}`} position={[xOffset, londonLandmarks.bigBen.parliamentHeight + 1.5, 0]}>
          ...
          {/* Tower Bridge */}
          <group position={[londonLandmarks.towerBridge.x, 0, londonLandmarks.towerBridge.z]}>
            {/* Left tower */}
            <mesh position={[-15, londonLandmarks.towerBridge.towerHeight / 2, 0]}>
              <boxGeometry args={[londonLandmarks.towerBridge.towerWidth, londonLandmarks.towerBridge.towerHeight, londonLandmarks.towerBridge.towerWidth]} />
              <meshStandardMaterial color={'#6b7280'} roughness={0.7} />
            </mesh>
            {/* Left tower turrets */}
            {[-1.5, 1.5].map((zOffset, i) => (
              <mesh key={`left-turret-${i}`} position={[-15, londonLandmarks.towerBridge.towerHeight + 2, zOffset]}>
            ...
            {/* Right tower turrets */}
            {[-1.5, 1.5].map((zOffset, i) => (
              <mesh key={`right-turret-${i}`} position={[15, londonLandmarks.towerBridge.towerHeight + 2, zOffset]}>
                <coneGeometry args={[1.2, 4, 8]} />
                <meshStandardMaterial color={'#1e3a5f'} roughness={0.6} />
              </mesh>
            ))}
            {/* Upper walkway */}
            <mesh position={[0, londonLandmarks.towerBridge.towerHeight - 2, 0]}>
              <boxGeometry args={[30, 1, 3]} />
              <meshStandardMaterial color={'#4a90a4'} metalness={0.3} roughness={0.5} />
            </mesh>
            {/* Lower bridge deck */}
            <mesh position={[0, londonLandmarks.towerBridge.bridgeHeight, 0]}>
              <boxGeometry args={[londonLandmarks.towerBridge.bridgeWidth, 0.5, 4]} />
              <meshStandardMaterial color={'#6b7280'} roughness={0.7} />
            </mesh>
            {/* Bridge suspension cables (simplified) */}
            {[-20, -10, 10, 20].map((xPos, i) => (
              <mesh key={`cable-${i}`} position={[xPos, (londonLandmarks.towerBridge.towerHeight + londonLandmarks.towerBridge.bridgeHeight) / 2, 0]}>
                <cylinderGeometry args={[0.1, 0.1, londonLandmarks.towerBridge.towerHeight - londonLandmarks.towerBridge.bridgeHeight, 8]} />
          ...
          {/* London Eye */}
          <group position={[londonLandmarks.londonEye.x, 0, londonLandmarks.londonEye.z]}>
            {/* Main wheel */}
            <mesh position={[0, londonLandmarks.londonEye.radius + 2, 0]} rotation={[Math.PI / 2, 0, 0]}>
              <torusGeometry args={[londonLandmarks.londonEye.radius, 0.3, 8, 32]} />
              <meshStandardMaterial color={'#f8fafc'} metalness={0.4} roughness={0.4} />
            </mesh>
            {/* Wheel spokes */}
            {Array.from({ length: londonLandmarks.londonEye.spokeCount }, (_, i) => {
              const angle = (i / londonLandmarks.londonEye.spokeCount) * Math.PI * 2;
              return (
                <mesh 
                  key={`spoke-${i}`} 
                  position={[
                    0,
                    londonLandmarks.londonEye.radius + 2 + Math.sin(angle) * londonLandmarks.londonEye.radius * 0.5,
                    Math.cos(angle) * londonLandmarks.londonEye.radius * 0.5
                  ]}
                  rotation={[angle, 0, 0]}
                >
                  <cylinderGeometry args={[0.08, 0.08, londonLandmarks.londonEye.radius, 4]} />
                  <meshStandardMaterial color={'#d1d5db'} roughness={0.5} />
                </mesh>
              );
            })}
            {/* Support structure (A-frame) */}
            <mesh position={[-2, londonLandmarks.londonEye.radius / 2 + 1, 0]} rotation={[0, 0, 0.3]}>
              <cylinderGeometry args={[0.4, 0.6, londonLandmarks.londonEye.radius + 4, 8]} />
          ...
          {/* The Shard */}
          <group position={[londonLandmarks.theShard.x, 0, londonLandmarks.theShard.z]}>
            {/* Main pyramidal structure */}
            <mesh position={[0, londonLandmarks.theShard.height / 2, 0]}>
              <coneGeometry args={[londonLandmarks.theShard.baseWidth, londonLandmarks.theShard.height, 4]} />
              <meshStandardMaterial color={'#94a3b8'} metalness={0.6} roughness={0.2} />
            </mesh>
            {/* Glass panels effect (vertical lines) */}
            {[-1, 0, 1].map((offset, i) => (
              <mesh key={`shard-panel-${i}`} position={[offset * 1.2, londonLandmarks.theShard.height / 2, londonLandmarks.theShard.baseWidth / 2 + 0.01]}>
                <boxGeometry args={[0.1, londonLandmarks.theShard.height * 0.9, 0.1]} />
          ...
          {/* St Paul's Cathedral */}
          <group position={[londonLandmarks.stPauls.x, 0, londonLandmarks.stPauls.z]}>
            {/* Main building */}
            <mesh position={[0, londonLandmarks.stPauls.buildingHeight / 2, 0]}>
              <boxGeometry args={[londonLandmarks.stPauls.buildingWidth, londonLandmarks.stPauls.buildingHeight, 8]} />
              <meshStandardMaterial color={'#f5f5f4'} roughness={0.7} />
            </mesh>
            {/* Dome base */}
            <mesh position={[0, londonLandmarks.stPauls.buildingHeight + 1, 0]}>
              <cylinderGeometry args={[londonLandmarks.stPauls.domeRadius + 1, londonLandmarks.stPauls.domeRadius + 2, 2, 16]} />
              <meshStandardMaterial color={'#f5f5f4'} roughness={0.7} />
            </mesh>
            {/* Main dome */}
            <mesh position={[0, londonLandmarks.stPauls.buildingHeight + 5, 0]}>
              <sphereGeometry args={[londonLandmarks.stPauls.domeRadius, 16, 12, 0, Math.PI * 2, 0, Math.PI / 2]} />
              <meshStandardMaterial color={'#6b7280'} roughness={0.5} />
            </mesh>
            {/* Lantern on top */}
            <mesh position={[0, londonLandmarks.stPauls.buildingHeight + londonLandmarks.stPauls.domeRadius + 5, 0]}>
              <cylinderGeometry args={[0.8, 1, 3, 8]} />
              <meshStandardMaterial color={'#f5f5f4'} roughness={0.6} />
            </mesh>
            {/* Cross on top */}
            <mesh position={[0, londonLandmarks.stPauls.buildingHeight + londonLandmarks.stPauls.domeRadius + 7.5, 0]}>
              <boxGeometry args={[0.2, 2, 0.2]} />
              <meshStandardMaterial color={'#c9a227'} roughness={0.4} />
            </mesh>
            <mesh position={[0, londonLandmarks.stPauls.buildingHeight + londonLandmarks.stPauls.domeRadius + 8, 0]}>
              <boxGeometry args={[1.2, 0.2, 0.2]} />
              <meshStandardMaterial color={'#c9a227'} roughness={0.4} />
            </mesh>
            {/* Front columns */}
            {[-3, -1.5, 0, 1.5, 3].map((xPos, i) => (
              <mesh key={`stpauls-column-${i}`} position={[xPos, londonLandmarks.stPauls.buildingHeight / 2, 4.5]}>
                <cylinderGeometry args={[0.4, 0.4, londonLandmarks.stPauls.buildingHeight - 1, 8]} />
                <meshStandardMaterial color={'#f5f5f4'} roughness={0.6} />
              </mesh>
            ))}
          </group>
          ...
          {/* City buildings */}
          {londonLandmarks.cityBuildings.map((bldg, idx) => (
            <mesh key={`london-bldg-${idx}`} position={[bldg.x, bldg.height / 2, bldg.z]}>
              <boxGeometry args={[bldg.width, bldg.height, bldg.depth]} />
          ...
          {londonLandmarks.bridges.map((bridge, idx) => (
            <group key={`london-bridge-${idx}`} position={[bridge.x, 0, bridge.z]}>
              {/* Bridge deck */}
              <mesh position={[0, bridge.height, 0]}>
                <boxGeometry args={[bridge.width, 0.5, 3]} />
                <meshStandardMaterial color={'#6b7280'} roughness={0.7} />
